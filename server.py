...
class Server(object):
    ...
    * self.room: one of 0, 1, 2, 3. This signifies which "room" the client is in,
    ...
    def room_description(self, room_number):
        ...
        return [
            'You are in the room with the polar bear.',
            'You are in the room with the green monster.',
            'You are in the room with the red dragon.',
            'You are in the room with the yellow dragonfly.',
        ][room_number]
    ...
    def route(self):
        ...
        # if 'self.input_buffer' was "say Is anytbody here?"", then:
        ...
        # routing dictionary
        # notice no parenthesis after the methods because
        # we don't want to invoke them but rather reference them
        {
            'quit': self.quit,
            'move': self.move,
            'say': self.say,
